...
from frontier_assurance.receipt import reproduce_receipt, verify_receipt, verify_receipt_inputs
...
def test_receipt_v2_runtime_rejects_empty_assurance_sections(tmp_path):
    for section in ("code", "inputs", "outputs", "checks"):
        case_root = tmp_path / section
        case_root.mkdir()
        dst = case_root / "receipt-demo"
        shutil.copytree(RECEIPT.parent, dst)
        receipt_path = dst / "receipt.yaml"
        doc = load_structured(receipt_path)
        doc[section] = []
...
def test_legacy_v1_verifies_nonexecuting_but_fresh_reproduction_refuses_it(tmp_path):
    dst = tmp_path / "receipt-demo"
    shutil.copytree(RECEIPT.parent, dst)
    receipt_path = dst / "receipt.yaml"
    doc = load_structured(receipt_path)
    doc["receipt_version"] = "1.0"
    doc["experiment"].pop("entrypoint", None)
    doc.pop("code", None)
    receipt_path.write_text(yaml.safe_dump(doc, sort_keys=False), encoding="utf-8")
    ...
    reproduced = reproduce_receipt(receipt_path, timeout=30)
    assert not reproduced.ok
    assert any("requires receipt_version 2.0" in error for error in reproduced.errors)
